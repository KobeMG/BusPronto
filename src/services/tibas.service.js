...
/**
 * Fetches all available stops for the tibas route from the database.
 * @returns {Promise<Array>} A list of stops ordered by name.
 */
export const getTibasStops = async () => {
    try {
        const { data, error } = await supabase
            .from('v_salidas_tibas')
            .select('*')
            .order('nombre_parada');
        ...
        console.error('Error in getTibasStops:', err);
        throw err;
    }
};

/**
 * Fetches the schedule for a given tibas stop, including days and fares.
 * @param {string} stopId Internal or UUID stop ID. 
 * @returns {Promise<Object>} The stop record with fully formatted schedules.
 */
export const getTibasStopDetails = async (stopId) => {
    try {
        ...
        console.error('Error in getTibasStopDetails:', err);
        throw err;
    }
};